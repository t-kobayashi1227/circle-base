...
import { profile } from "@/lib/mypage-mock-data";
...
export function ProfileCard() {
  return (
    <div className="overflow-hidden rounded-xl border border-cb-border bg-cb-surface lg:min-w-0">
      {/* デスクトップ: カバー写真の上に名前・属性・自己紹介を重ねる */}
      <div className="relative hidden h-[132px] lg:block">
        <PhotoPlaceholder caption="カバー写真：夕暮れの新潟の街並み" iconSize={20} />
        ...
        <div className="pointer-events-none absolute bottom-4 left-[150px]">
          <div className="flex items-center gap-[11px]">
            <span className="font-heading text-xl font-bold text-white">{profile.displayName}</span>
            <span className="rounded px-2.5 py-1 text-[10.5px] font-medium text-cb-ink-soft" style={{ background: "rgba(255,255,255,.92)" }}>
              {profile.areaBadge}
            </span>
          </div>
          <div className="mt-2 text-[11px] text-white/90">{profile.ageGender}</div>
          <div className="mt-1.5 text-[11px] text-white/90">{profile.bio}</div>
        </div>
      </div>
      ...
      <div className="relative hidden py-3.5 pl-[142px] pr-5 lg:block">
        ...
        <div className="flex items-center gap-[15px] text-[11.5px] text-cb-muted">
          <StatItem icon="group" iconColor="#A79D8E" label="参加サークル" value={profile.joinedCount} />
          <span className="h-4 w-px bg-[#EFE7DA]" />
          <StatItem icon="groups" iconColor="#A79D8E" label="主催サークル" value={profile.ownedCount} />
          <span className="h-4 w-px bg-[#EFE7DA]" />
          <StatItem icon="thumb_up" iconColor="#C99A3E" label="もらったいいね" value={profile.likesCount} />
        </div>
      </div>
      ...
      <div className="lg:hidden">
        <div className="relative h-[104px]">
          <PhotoPlaceholder caption="カバー写真：新潟の街並み" iconSize={16} />
        </div>
        <div className="relative pl-32 pr-4 pt-2.5">
          <div className="absolute left-4 top-[-50px] h-24 w-24 overflow-hidden rounded-full border-4 border-white shadow-[0_4px_14px_rgba(60,45,20,.16)]">
            <PhotoPlaceholder caption="プロフィール写真" iconSize={14} />
          </div>
          <div className="flex items-center gap-2.5">
            <span className="font-heading text-lg font-bold text-[#2F2B24]">{profile.displayName}</span>
            <span className="whitespace-nowrap rounded border border-[#E6DCCB] px-2.5 py-1 text-[10px] text-cb-ink-soft">
              {profile.areaBadge}
            </span>
          </div>
        </div>
        <div className="pl-32 pr-4 pt-3">
          <div className="text-[11.5px] text-cb-ink-soft">{profile.ageGender}</div>
          <div className="mt-1.5 text-[11.5px] leading-[1.7] text-cb-ink-soft">{profile.bioShort}</div>
        </div>
        <div className="mt-4 grid grid-cols-3 gap-2 border-t border-[#F3ECE0] px-3 pb-4 pt-3.5">
          <div className="flex flex-col items-center gap-1.5">
            ...
            <span className="font-heading text-lg font-bold text-[#2F2B24]">{profile.joinedCount}</span>
          </div>
          <div className="flex flex-col items-center gap-1.5">
            <span className="flex items-center gap-1 text-[10.5px] text-cb-muted-2">
            ...
            <span className="font-heading text-lg font-bold text-[#2F2B24]">{profile.ownedCount}</span>
          </div>
          <div className="flex flex-col items-center gap-1.5">
            <span className="flex items-center gap-1 text-[10.5px] text-cb-muted-2">
            ...
            <span className="font-heading text-lg font-bold text-[#2F2B24]">{profile.likesCount}</span>
          </div>
        </div>
      </div>
    </div>
