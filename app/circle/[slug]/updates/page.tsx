...
import { ActivityGallery } from "@/components/circle-detail/activity-gallery";
...
export async function generateMetadata({
  ...
  return { title: `活動の様子 | ${data.name}` };
}

export default async function CircleUpdatesTabPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col bg-cb-bg text-cb-ink">
      ...
      <main className="flex-1">
        ...
        <TabNav slug={circle.slug} active="updates" updatesCount={circle.updatesCount} />
