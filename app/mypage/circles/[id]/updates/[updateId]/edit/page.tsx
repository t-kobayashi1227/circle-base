...
import { ActivityEditForm } from "@/components/circle-update-post/activity-edit-form";
import { PostSidebar } from "@/components/circle-update-post/post-sidebar";
import { SiteFooter } from "@/components/home/site-footer";
import { MobileBottomNav } from "@/components/home/mobile-bottom-nav";
import { getOwnedCircleById, getCircleUpdateById, sortedUpdateImages } from "@/lib/circles";
import { getCurrentUser } from "@/lib/auth";

export default async function EditActivityPostPage({
  params,
}: {
  ...
  if (!circle || !update || update.kind !== "activity") notFound();

  const managePath = `/mypage/circles/${id}`;

  return (
    <div className="flex min-h-full flex-1 flex-col bg-cb-bg text-cb-ink">
      <WizardHeader title="活動の様子を編集" backHref={`${managePath}/updates`} closeHref={`${managePath}/updates`} />
      ...
      <main className="flex-1">
        ...
        <div className="px-7 pb-2 pt-[26px]">
          <div className="hidden lg:block">
            <h1 className="font-heading text-[25px] font-bold text-[#2F2B24]">活動の様子を編集</h1>
        ...
        <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_320px] lg:items-start lg:gap-[22px] lg:px-7 lg:pb-[34px]">
          <div className="mx-3 my-3.5 rounded-xl border border-cb-border bg-cb-surface px-4 py-[18px] lg:mx-0 lg:my-0 lg:px-[26px] lg:py-6">
            <ActivityEditForm
              ...
              defaultImages={sortedUpdateImages(update)}
