...
export default async function CirclesPage({
  ...
  const [{ circles, total, pageCount }, categoryTree, areas, user] = await Promise.all([
    getCircles({ categorySlug: sp.category, areaSlug: sp.area, keyword: sp.q, sort, page }),
    getCategoryTree(),
    getAreas(),
    getCurrentUser(),
  ]);
      ...
      <main className="flex-1">
        ...
        <div className="px-[18px] pt-3.5 lg:grid lg:grid-cols-[250px_minmax(0,1fr)] lg:items-start lg:gap-[26px] lg:px-7 lg:pt-1.5 lg:pb-[34px]">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-[9px] text-[11.5px] text-cb-muted-2 lg:text-[12.5px]">{total}件のサークルが見つかりました</p>
            ...
            <SortBar current={sp} sort={sort} />

            {circles.length === 0 ? (
              <p className="mt-10 text-center text-[12.5px] text-cb-muted">
                条件に一致するサークルが見つかりませんでした。条件を変えて検索してみてください。
              </p>
            ) : (
